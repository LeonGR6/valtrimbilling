...
const STATUS_OPTIONS = ['Confirmado', 'En curso', 'Completado', 'Excepción']
...
function EventCard({ event, timeText }) {
  ...
  return (
    <Box className={`work-event work-event--${statusTone[status] ?? 'confirmed'}`}>
  ...
  return (
    <Box className="event-detail">
      ...
      <Box className="event-detail__title">
        <Box className={`event-detail__code event-detail__code--${statusTone[props.status]}`}>{props.code}</Box>
        <Box>
      ...
      <Chip className={`status-chip status-chip--${statusTone[props.status]}`} label={props.status} size="small" />
...
export default function CalendarPage() {
  ...
  const saveEvent = (event) => {
    ...
    if (new Date(end) <= new Date(start)) {
      setNotice('La hora de fin debe ser posterior a la hora de inicio.')
      return
    }

    const eventData = {
      id: editingId ?? `evt-${Date.now()}`,
      title: `${draft.code} • ${getLotsLabel(draft.lots)}`,
      start,
      end,
      extendedProps: {
        code: draft.code,
        ...
        crew: draft.crew,
        plan: draft.plan,
        progress: draft.status === 'Completado' ? 100 : (draft.progress ?? 0),
        rate: Number(draft.rate) || 0,
        billingReady: draft.status === 'Completado' ? (draft.billingReady ?? true) : false,
      },
    }
          ...
          <Box className="calendar-canvas">
            <FullCalendar
              ...
              eventClassNames={({ event }) => [
                `fc-work-event--${statusTone[event.extendedProps.status] ?? 'confirmed'}`,
