...
const responseLinks = {
  confirmed:
    "https://billing.valtrim.com/follow-up/respond?token=signed&intent=confirmed",
  notReady:
    "https://billing.valtrim.com/follow-up/respond?token=signed&intent=not-ready",
};
...
Deno.test("renders a deterministic Builder follow-up without treating it as confirmation", () => {
  ...
  assertMatch(
    rendered.text,
    /immediately updates the official Production date/u,
  );
  assertMatch(rendered.html, /Jamie &lt;Superintendent&gt;/u);
  assertMatch(rendered.html, /Production confirmation requested/u);
  assertMatch(rendered.html, /Confirm scheduled date/u);
  assertMatch(rendered.html, /Not ready — choose a new date/u);
  assertMatch(
    rendered.html,
    /Four-week follow-up · 28 days before scheduled work/u,
  );
...
Deno.test("renders an internal no-response escalation without changing the schedule", () => {
  ...
  assertEquals(
    rendered.subject,
    "Needs attention: no response for Exterior Frames • JOB-01",
  );
  ...
  assertMatch(rendered.html, /No Jobsite response received/u);
  assertMatch(rendered.html, /Scheduling attention required/u);
  assertMatch(rendered.html, /Review required/u);
});
...
Deno.test("renders Jobsite acknowledgement and internal date-change notices", () => {
  ...
  assertEquals(
    receipt.subject,
    "Updated: Exterior Frames for JOB-01 moved to 2026-10-22",
  );
  assertMatch(
    receipt.text,
  ...
  assertMatch(receipt.html, /Your new date was recorded/u);
  assertMatch(receipt.html, /Previous date/u);
  assertMatch(receipt.html, /New official date/u);
  ...
  assertEquals(
    internal.subject,
    "Schedule changed by Jobsite: Exterior Frames • JOB-01",
  );
  assertMatch(internal.text, /no approval is required/u);
  assertMatch(internal.html, /Production date changed/u);
  assertMatch(internal.html, /No approval is required/u);

  const confirmation = renderBuilderFollowUpResponseEmail({
    ...responseSnapshot,
    responseAction: "CONFIRMED",
    finalWorkDate: responseSnapshot.targetWorkDate,
    reason: null,
  });
  assertMatch(confirmation.html, /Schedule confirmed/u);
  assertMatch(confirmation.html, /Your response was received/u);
  assertMatch(confirmation.html, /Confirmed production date/u);
});
